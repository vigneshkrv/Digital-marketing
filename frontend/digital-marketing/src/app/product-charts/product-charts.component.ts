...
@Component({
  ...
  styleUrls: ['./product-charts.component.scss']
})
export class ProductChartsComponent implements OnInit {
  ...
  ngOnInit() {
    this.apiService.getDashboardData2().subscribe(x => {
      this.reviews = x
      console.log(x)
      for (var i = 0; i < this.reviews.length; i++) {
        this.rating[i]=parseInt(this.reviews[i].rating)
        this.ratingArr[this.rating[i]]++;
      }
  ...
  private initSvg() {
    ...
    this.svg = d3.select('#product_rating');
