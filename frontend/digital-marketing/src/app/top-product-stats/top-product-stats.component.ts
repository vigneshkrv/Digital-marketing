...
@Component({
  selector: 'app-top-product-stats',
  templateUrl: './top-product-stats.component.html',
  styleUrls: ['./top-product-stats.component.scss']
})
export class TopProductStatsComponent implements OnInit,AfterViewInit {
@Input() product:any
@Input() Pid: any
reviews: any = {ratings:[]}
...
ngOnInit() {
    ...
    this.reviews.ratings = [0,1,2,3,4];
    for (var i = 0; i <= 4; i++) {
      this.ratingArr[i]=this.product[`rate${i+1}`];
      console.log(this.ratingArr[i]);
    }
    this.last2 = this.product.asin.slice(8,10)
...
ngAfterViewInit() {
  ...
  this.createBarChart(this.reviews.ratings);
}
...
private init_X_Y_Axis() {
  ...
  this.x.domain(this.ratings.map((d) => Number(d)));
...
private createBarChart(data) {
  ...
  bars.enter().append('rect')
    .attr('class', 'bar')
    .attr('x', (d) => this.x(d+1))
    .attr('y', (d) => this.y(this.ratingArr[d]))
    .attr('width', this.x.bandwidth())
    .attr('height', (d) => {
      console.log(this.ratingArr[d])
      // console.log(this.height - this.y(this.ratingArr[d.rating]))
      return this.height - this.y(this.ratingArr[d]);
    });
  bars.enter()
    .append("text")
    .attr("class", "bar")
    .attr("text-anchor", "middle")
    .attr('x', (d) => this.x(d+1)+5)
    .attr('y', (d) => {
      return this.y(this.ratingArr[d]) - 5
    })
    .text((d) => {
      return this.ratingArr[d]
    });
